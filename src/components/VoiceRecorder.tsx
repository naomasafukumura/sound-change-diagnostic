...
export function VoiceRecorder({ expectedSentence }: VoiceRecorderProps) {
  ...
  if (state === 'done') {
    return (
      <div className="animate-fade-in rounded-xl border border-[#e2e8f0] overflow-hidden">
        ...
        <div className="px-4 py-3 border-t border-[#e2e8f0]">
          ...
          <p className="font-medium text-sm">{expectedSentence}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {state === 'recording' ? (
        <button
          onClick={handleStop}
          className="flex items-center justify-center gap-2 w-full py-3 bg-red-500 text-white rounded-xl text-sm font-bold active:scale-[0.98] transition-transform"
        >
          <span className="w-2 h-2 rounded-full bg-white animate-rec" />
          タップして停止
        </button>
      ) : state === 'processing' ? (
        ...
        <button
          onClick={handleStart}
          className="flex items-center justify-center gap-2 w-full py-3 border border-[#e2e8f0] text-[#64748b] rounded-xl text-sm font-medium active:scale-[0.98] transition-transform hover:border-[#94a3b8] hover:text-[#475569]"
        >
